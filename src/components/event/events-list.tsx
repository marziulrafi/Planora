...
import { filterEventsByTimeframe, sortEventsByTimeframe } from "@/src/lib/event-dates";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const endpoint = `/events?${params.toString()}&timeframe=${timeframe}`;

        const result = await api.get<PaginatedEvents>(endpoint);
        const filtered = sortEventsByTimeframe(
          filterEventsByTimeframe(result.data, timeframe),
          timeframe
        );
    ...
    void load();
  }, [debouncedQuery, filterIdx, page, timeframe]);
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <p className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">
          {emptyMessage}
        </p>
      ) : (
